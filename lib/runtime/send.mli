...
(** [show ctx window] makes river show [window].
    ...
    {b Effects:} sends River request *)
val show : Ctx.render Ctx.t -> Oxbow_state.Window.t -> unit

(** [hide ctx window] makes river hide [window].

    {b Effects:} sends River request *)
val hide : Ctx.render Ctx.t -> Oxbow_state.Window.t -> unit

(** [close ctx window] asks [window] to close.

    {b Effects:} sends River request *)
val close : Ctx.manage Ctx.t -> Oxbow_state.Window.t -> unit
...
val fullscreen
  :  Ctx.manage Ctx.t
  -> Oxbow_state.Window.t
  -> output:Oxbow_state.Output.t
  -> unit

(** [exit_fullscreen ctx window] takes [window] out of the fullscreen state.

    {b Effects:} sends River request *)
val exit_fullscreen : Ctx.manage Ctx.t -> Oxbow_state.Window.t -> unit

(** [inform_fullscreen ctx window] tells [window] it is fullscreen.

    {b Effects:} sends River request *)
val inform_fullscreen : Ctx.manage Ctx.t -> Oxbow_state.Window.t -> unit

(** [inform_not_fullscreen ctx window] tells [window] it is not fullscreen.

    {b Effects:} sends River request *)
val inform_not_fullscreen : Ctx.manage Ctx.t -> Oxbow_state.Window.t -> unit

(** [inform_maximized ctx window] tells [window] it is maximized.

    {b Effects:} sends River request *)
val inform_maximized : Ctx.manage Ctx.t -> Oxbow_state.Window.t -> unit

(** [inform_unmaximized ctx window] tells [window] it is not maximized.

    {b Effects:} sends River request *)
val inform_unmaximized : Ctx.manage Ctx.t -> Oxbow_state.Window.t -> unit

(** [inform_resize_start ctx window] tells [window] an interactive resize
    started.

    {b Effects:} sends River request *)
val inform_resize_start : Ctx.manage Ctx.t -> Oxbow_state.Window.t -> unit

(** [inform_resize_end ctx window] tells [window] the interactive resize ended.

    {b Effects:} sends River request *)
val inform_resize_end : Ctx.manage Ctx.t -> Oxbow_state.Window.t -> unit

(** [set_capabilities ctx window ~caps] sets the capabilities of [window].

    {b Effects:} sends River request *)
val set_capabilities : Ctx.manage Ctx.t -> Oxbow_state.Window.t -> caps:int32 -> unit

(** [set_tiled ctx window ~edges] marks the tiled edges of [window].

    {b Effects:} sends River request *)
val set_tiled : Ctx.manage Ctx.t -> Oxbow_state.Window.t -> edges:int32 -> unit

(** [use_csd ctx window] asks [window] to draw client-side
    decorations.

    {b Effects:} sends River request *)
val use_csd : Ctx.manage Ctx.t -> Oxbow_state.Window.t -> unit

(** [use_ssd ctx window] asks river to draw server-side decorations on [window].

    {b Effects:} sends River request *)
val use_ssd : Ctx.manage Ctx.t -> Oxbow_state.Window.t -> unit

(** [focus_window ctx seat window] gives [window] the keyboard focus of [seat].

    {b Effects:} sends River request *)
val focus_window : Ctx.manage Ctx.t -> Oxbow_state.Seat.t -> Oxbow_state.Window.t -> unit

(** [clear_focus ctx seat] clears the keyboard focus of [seat].

    {b Effects:} sends River request *)
val clear_focus : Ctx.manage Ctx.t -> Oxbow_state.Seat.t -> unit

(** [op_start_pointer ctx seat] starts a pointer operation on [seat].

    {b Effects:} sends River request *)
val op_start_pointer : Ctx.manage Ctx.t -> Oxbow_state.Seat.t -> unit

(** [op_end ctx seat] ends the pointer operation on [seat].

    {b Effects:} sends River request *)
val op_end : Ctx.manage Ctx.t -> Oxbow_state.Seat.t -> unit

(** [pointer_warp ctx seat ~x ~y] warps the pointer of [seat] to ([x], [y]).

    {b Effects:} sends River request *)
val pointer_warp : Ctx.manage Ctx.t -> Oxbow_state.Seat.t -> x:int32 -> y:int32 -> unit

(** [set_position ctx window ~x ~y] positions the node of [window] at
    ([x], [y]).

    {b Effects:} sends River request *)
val set_position : Ctx.render Ctx.t -> Oxbow_state.Window.t -> x:int32 -> y:int32 -> unit
...
(** [place_top ctx window] raises the node of [window].
    ...
    {b Effects:} sends River request *)
val place_top : Ctx.render Ctx.t -> Oxbow_state.Window.t -> unit
...
(** [modifiers_watch ctx seat] sends the watch request for [seat] when the
    ...
    {b Effects:} sends River request *)
val modifiers_watch : Ctx.manage Ctx.t -> Oxbow_state.Seat.t -> unit
