...
let compute ~(params : Params.Tiling.t) ~(usable_area : int Rect.t) ~(count : int) =
  ...
  | n ->
    ...
    let clients =
      ...
      | Even ->
        column ~x:c_area.x ~w:cw ~y:c_area.y (split ~total:c_area.h ~count:c_count)
      | Diminish ->
        column ~x:c_area.x ~w:cw ~y:c_area.y (diminish_heights ~total:c_area.h c_count)
      | Dwindle ->
        if c_count = 0 then [] else subdivide ~pick:dwindle_pick c_area 0 c_count
      | Spiral -> if c_count = 0 then [] else subdivide ~pick:spiral_pick c_area 0 c_count
    in
    masters @ clients
;;
