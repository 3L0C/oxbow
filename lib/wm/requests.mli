...
val window_request : Ctx.manage Ctx.t -> Window.t -> Window_request.t -> unit

(** [focus_request ctx seat] handles the focus request on [seat].

    {b Effects:} mutates WM state; sends River request *)
val focus_request : Ctx.manage Ctx.t -> Seat.t -> unit

(** [interaction ctx seat] handles the interaction request on [seat].

    {b Effects:} mutates WM state; sends River request *)
val interaction : Ctx.manage Ctx.t -> Seat.t -> unit
...
val action : Ctx.manage Ctx.t -> Seat.t -> Action.t -> unit
