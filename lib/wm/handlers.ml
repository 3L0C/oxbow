...
let on_new_seat (ctx : Ctx.manage Ctx.t) (seat : Seat.t) =
...
let set_presentation_mode (output : Output.t) =
...
let render (ctx : Ctx.render Ctx.t) (seat : Seat.t) =
...
let manage_seat (ctx : Ctx.manage Ctx.t) (seat : Seat.t) =
...
let manage_window (ctx : Ctx.manage Ctx.t) (window : Window.t) =
...
let disconnect_seat (window : Window.t) (seat : Seat.t) =
...
let disconnect_seats (window : Window.t) = List.iter (disconnect_seat window)
...
let close_seats (wm : Window_manager.t) =
  wm.seats
  <- List.filter
       (fun (s : Seat.t) ->
          match s.state with
...
let manage_output (ctx : Ctx.manage Ctx.t) (output : Output.t) =
...
let on_manage_start proxy (wm_box : Window_manager.t Box.t) =
...
let on_render_start proxy (wm_box : Window_manager.t Box.t) =
  let wm = Option.get wm_box.body in
...
let on_output _ river_output (wm_box : Window_manager.t Box.t) =
  let wm = Option.get wm_box.body in
  ...
  let layer_shell =
    ...
    @@ object
         ...
         method user_data = Types.Boxed_data (Output_box output_box)

         method on_non_exclusive_area proxy ~x ~y ~width ~height =
           match Wayland.Proxy.user_data proxy with
           | Types.Boxed_data (Output_box { body = Some o }) ->
             o.usable
...
let on_seat _ river_seat (wm_box : Window_manager.t Box.t) =
  let wm = Option.get wm_box.body in
