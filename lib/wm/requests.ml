...
let focus_request (ctx : Ctx.manage Ctx.t) (seat : Seat.t) =
  let wm = Ctx.wm ctx in
...
let interaction (ctx : Ctx.manage Ctx.t) (seat : Seat.t) =
...
let action (ctx : Ctx.manage Ctx.t) (seat : Seat.t) (action : Action.t) =
  let wm = Ctx.wm ctx in
  match action with
  | No_action -> ()
    ...
    (match Focus.focused_of seat with
     ...
     | Some w ->
       (match w.presentation with
        | P_fullscreen _ -> ()
        | _ ->
          Window_request.(if w.is_maximized then Req_unmaximize else Req_maximize)
