...
module Pointer_binding = Types.Pointer_binding
module Xkb_binding = Types.Xkb_binding
open Ocdwm_core.Types
open Ocdwm_ipc.Types

let xkb_binding_destroy (binding : Xkb_binding.t) =
...
let pointer_binding_destroy (pointer : Pointer_binding.t) =
...
let handle_pointer_position (wm : Window_manager.t) (s : t) ~(x : int32) ~(y : int32) =
  s.position <- { x; y };
...
let refresh_cursor_target (wm : Window_manager.t) (s : t) =
  s.cursor_target
  <- (match s.output with
